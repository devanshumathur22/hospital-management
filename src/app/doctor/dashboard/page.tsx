...
export default function DoctorDashboard(){
  ...
  /* 🔥 NOTIFICATIONS */
  const notifications = [
    ...safeAppointments.slice(0,2).map(a=>({
      type:"new",
      text:`New appointment from ${a.patient?.name || "Patient"}`
    })),
    ...safeAppointments
      ...
      .map(a=>({
        type:"cancel",
        text:`Cancelled by ${a.patient?.name || "Patient"}`
      }))
  ]
    ...
    <div className="p-4 sm:p-6 md:p-8 space-y-6 md:space-y-10 bg-gray-50 min-h-screen">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-blue-100 p-4 rounded-xl shadow">
          ...
          <h2 className="text-2xl font-bold">{safeAppointments.length}</h2>
        </div>
      ...
      <div className="bg-white p-6 rounded-xl shadow">
        ...
        <div className="space-y-2 text-sm">
          ...
          {safePrescriptions.slice(0,5).map((p:any)=>(
            <div key={p.id} className="flex justify-between border-b pb-1">

              <span>{p.patient?.name || "Patient"}</span>
